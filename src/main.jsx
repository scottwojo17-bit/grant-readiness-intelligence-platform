...
function formatFunderName(value) {
  ...
  return acronymMap[normalized.toLowerCase()] || titleCase(normalized || "Unknown funder");
}

function formatOrganizationName(value, fallback) {
  const normalized = String(value || fallback || "").trim();
  const knownNames = {
    veteransforge: "Veterans Forge",
    veteransforgeorg: "Veterans Forge",
  };
  const compact = normalized.toLowerCase().replace(/[^a-z0-9]/g, "");
  return knownNames[compact] || titleCase(normalized.replace(/([a-z])([A-Z])/g, "$1 $2"));
}
...
function createGrantProfile(url) {
  try {
    ...
    return {
      ...
      funder: formatFunderName(hostname.split(".")[0] || "Unknown funder"),
...
function extractHostnameName(url, fallback) {
  try {
    ...
    return formatOrganizationName(hostname.split(".")[0], fallback);
  } catch {
    return fallback;
  }
}
...
function summarizeOrganizationPool(organizations, grantProfile) {
  ...
  return `${organizations.length} organization profile(s) are mapped to ${grantProfile.grantName}. Average preliminary partner alignment is ${averageScore}%. ${strongest.orgName} is currently the strongest preliminary fit at ${strongest.alignmentScore}% as ${strongest.role}. ${riskCount ? `${riskCount} organization(s) need backup documents or role justification before submission.` : "No organization is currently below the 70% partner threshold."}`;
}
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-[0.8fr_1.2fr]">
        <div className="rounded-lg border border-slateLine bg-slate-50 p-4">
          ...
          <div className="mt-3 grid gap-2 text-sm text-slate-600">
            ...
            <CardRow label="Preliminary Organization Fit" value={`${alignment.total}%`} />
...
function ScoreBreakdown({ analytics, alignment, collaboration }) {
  ...
  const scoreCards = [
    ...
    { label: "Preliminary Organization Fit", score: alignment.total, note: "Mission, geography, population, capabilities, and partner match before final verification." },
...
function GrantTrackerDatabase({ records, activeTrackerId, hasAnalyzed, onSaveCurrent, onLoad, onRemove, onClearAll }) {
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button
            ...
            onClick={() => {
              if (window.confirm("Clear all saved grant tracker records from this browser?")) {
                onClearAll();
              }
            }}
            type="button"
                  ...
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap gap-2">
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (window.confirm(`Remove ${record.grantName} from this browser tracker?`)) {
                            onRemove(record.id);
                          }
                        }}
                        type="button"
  ...
  return (
    <>
      ...
      <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-800">
        This client-side workspace does not scrape the live webpage yet. It creates draft grant-specific criteria and initial
        grades from the submitted URL until the backend parser is connected. Verify source evidence before using results in proposal language.
      </div>
    </>
  );
}
...
function IntelligenceEngine({ organizationIntel, alignment, extractorSummary }) {
  return (
    <section className="grid gap-6 xl:grid-cols-[1fr_1fr]">
      ...
      <div className="grid gap-6">
        <div className="panel">
          <SectionTitle icon={BarChart3} eyebrow="Grant Alignment Engine" title={`Preliminary alignment score: ${alignment.total}%`} />
          <div className="mt-5 grid gap-3">
          ...
          <table className="w-full min-w-[980px] text-left text-sm">
            <thead className="bg-slate-100 text-xs font-semibold uppercase tracking-wide text-slate-600">
              <tr>
                ...
                <th className="px-4 py-3">Preliminary Alignment</th>
  ...
  return (
    <section className="grid gap-6">
      ...
      <div className="panel">
        ...
        <div className="mt-4 rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-sm leading-6 text-blue-800">
          Upload requested documents for each requirement. This client-side review scans readable document text and file names in the browser,
