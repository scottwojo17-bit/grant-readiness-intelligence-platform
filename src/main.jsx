...
function App() {
  ...
  function analyzeGrant() {
    ...
    setUrlError("");
    setGrantProfile(createGrantProfile(submittedUrl));
    ...
    <main className="min-h-screen bg-slate-50 text-ink">
      ...
      <div className="mx-auto grid max-w-7xl gap-6 px-5 py-6">
        <section className="rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-sm leading-6 text-blue-800">
          Prototype generates a readiness profile from the submitted link in the browser. Full live page parsing will be added
          when the backend/API version is connected.
        </section>
  ...
  return (
    <>
      ...
      <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-800">
        This client-only prototype does not scrape the live webpage yet. It creates the analysis record from the submitted
        URL and applies the standard grant readiness criteria until the backend parser is connected.
      </div>
    </>
  );
}
