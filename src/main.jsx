...
function createOrganizationWebsiteRecord({ url, role, grantProfile, index = 0 }) {
  const seed = hashString(`${url}-${role}-${grantProfile.grantName}-${index}`);
  const orgName = extractHostnameName(url, role === "Lead Applicant" ? "Lead Organization" : `Sub Organization ${index + 1}`);
  ...
  const industry = industryOptions[seededValue(seed, 0, industryOptions.length - 1, 1)];
...
function scoreOrganizationEvidence(evidence) {
  const score = organizationAlignmentRubric.reduce((sum, [, points, key]) => {
    ...
    const earned = confidence === "Confirmed by website" ? points : confidence === "Likely but not confirmed" ? Math.round(points * 0.65) : confidence === "Needs human verification" ? Math.round(points * 0.4) : 0;
    return sum + earned;
  }, 0);
  const missingItems = organizationAlignmentRubric
    .filter(([, , key]) => evidence[key]?.confidence !== "Confirmed by website")
    .map(([label]) => label);
  const riskScore = Math.max(0, 100 - score);
  const status = score >= 90 ? "Strong partner" : score >= 80 ? "Good partner" : score >= 70 ? "Needs backup documents" : score >= 50 ? "Weak fit" : "Do not use without justification";
  return { alignmentScore: score, riskScore, missingItems, status };
}
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-ink">
      ...
      <div className="mx-auto grid max-w-7xl gap-6 px-5 py-6">
        ...
        {hasAnalyzed ? (
          <>
            <OrganizationWebsiteIntake grantProfile={grantProfile} organizations={organizationWebsites} setOrganizations={setOrganizationWebsites} />
            <GRVFSOverview />
...
function OrganizationWebsiteIntake({ grantProfile, organizations, setOrganizations }) {
          ...
          <div className="mt-5 grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
            <div className="rounded-lg border border-slateLine bg-white p-4">
              ...
              <div className="mt-4 rounded-lg border border-slateLine bg-slate-50 p-4">
                ...
                <p className="mt-2 text-sm leading-6 text-slate-600">
                  {selectedOrg.orgName} is a {selectedOrg.alignmentScore}% fit for {grantProfile.grantName} because its website profile supports {selectedOrg.industry.toLowerCase()} work, {selectedOrg.capabilities.toLowerCase()}, and {selectedOrg.serviceArea.toLowerCase()} reach. Missing or uncertain evidence should be verified before naming this organization in the final submission.
                </p>
              </div>
            </div>

            <div className="rounded-lg border border-slateLine bg-white p-4">
              <h3 className="text-lg font-semibold text-ink">Website evidence confidence</h3>
