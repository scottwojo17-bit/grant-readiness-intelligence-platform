...
function App() {
  ...
  const isSampleUrl = analysisRun?.url?.trim() === sampleGrantUrl;
    ...
    <main className="min-h-screen bg-slate-50 text-ink">
      ...
      <div className="mx-auto grid max-w-7xl gap-6 px-5 py-6">
        <section className="rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-sm leading-6 text-blue-800">
          Prototype uses sample extraction data. Live URL parsing will be added in the backend version. New URLs are captured
          for the demo record, but v1 still reuses the NSF TechAccess sample criteria until an API parser is connected.
        </section>
          ...
          <>
            <section className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
              <div className="panel">
                ...
                <GrantMeta analysisRun={analysisRun} isSampleUrl={isSampleUrl} />
...
function GrantMeta({ analysisRun, isSampleUrl }) {
  return (
    <>
      <div className="mt-5 grid gap-3 rounded-lg border border-slateLine bg-slate-50 p-4 text-sm md:grid-cols-3">
        ...
        <Meta label="Submitted URL" value={analysisRun?.url || sampleGrantUrl} />
        <Meta label="Analysis Run" value={analysisRun?.analyzedAt || "Not run"} />
        <Meta label="Parser Status" value={isSampleUrl ? "Matched sample URL" : "Live parsing pending"} />
      </div>
      {!isSampleUrl ? (
        <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-800">
          This URL was recorded, but the displayed criteria are still the NSF sample extraction. Connect the backend parser
          to regenerate grant-specific requirements from new URLs.
        </div>
      ) : null}
    </>
  );
